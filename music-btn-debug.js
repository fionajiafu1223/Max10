...
`;
  ...
  function frSaveTracksToStorage() {
    ...
    } catch(e) { alert('DEBUG: storage写入失败 - ' + e.message); }
  }
  ...
  window.frSaveRecording = function() {
    ...
    reader.onload = function(e) {
      ...
      alert('DEBUG: base64生成成功，长度=' + base64.length);
      frImportedTracks.push({id:id, name:name, src:frRecordObjectUrl, base64:base64, mime:mime});
      alert('DEBUG: push后数组长度=' + frImportedTracks.length);
      frSaveTracksToStorage();
      alert('DEBUG: storage已保存，localStorage内容长度=' + (localStorage.getItem('freed_recorded_tracks')||'').length);
